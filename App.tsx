...
const TopBar: React.FC<TopBarProps> = ({ onMenuClick, onNotificationsClick }) => {
  ...
  useEffect(() => {
    ...
    const matchedProducts = products.filter(p => p.name.toLowerCase().includes(lowerQuery) || p.sku.toLowerCase().includes(lowerQuery)).slice(0, 3);
    const matchedCustomers = customers.filter(c => c.name.toLowerCase().includes(lowerQuery) || c.phone.includes(lowerQuery)).slice(0, 3);
    const matchedOrders = orders.filter(o => (o.id && o.id.toLowerCase().includes(lowerQuery)) || o.customerName.toLowerCase().includes(lowerQuery)).slice(0, 3);
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 transition-colors duration-300">
      ...
      {!isSuperAdmin ? (
        <div className="hidden md:flex flex-col relative w-full max-w-sm lg:max-w-md mx-4" ref={searchRef}>
          <div className="relative">
            ...
            />
            {searchQuery && (
              <button className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600" onClick={() => { setSearchQuery(''); setSearchResults({ products: [], customers: [], orders: [] }); }}>
                <X size={14} />
              </button>
            )}
          </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {!isSuperAdmin && (
          <button onClick={onNotificationsClick} className="relative p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors">
            <Bell size={20} />
            {unreadCount > 0 && <span className="absolute top-1.5 right-1.5 h-2.5 w-2.5 bg-red-500 rounded-full border-2 border-white dark:border-slate-800"></span>}
          </button>
        )}
        ...
        <div className="flex items-center gap-2">
           ...
           <button onClick={() => { logout(); }} className="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Logout">
             <LogOut size={18} />
           </button>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen transition-all duration-300 ml-0 w-full">
        <TopBar onMenuClick={() => setIsSidebarOpen(true)} onNotificationsClick={() => setIsNotificationsOpen(true)}/>
...
const DashboardIndex = () => {
  const { user } = useStore();
  if (user?.role === Role.SUPER_ADMIN) return <Navigate to="/app/admin/payments" replace />;
  return <DashboardStats />;
};

function App() {
  const { checkSession, isLoading } = useStore();
  ...
  // Show a global loading spinner while checking session to prevent flash of login page
  if (isLoading) {
      return (
    ...
    <HashRouter>
      <Routes>
        ...
        <Route path="/app" element={<ProtectedRoute><DashboardLayout /></ProtectedRoute>}>
          <Route index element={<ProtectedRoute><DashboardIndex /></ProtectedRoute>} />
          <Route path="pos" element={<ProtectedRoute requiredPermission="POS_ACCESS"><POSPage /></ProtectedRoute>} />
          <Route path="sales" element={<ProtectedRoute requiredPermission="MANAGE_ORDERS"><OrdersPage /></ProtectedRoute>} />
          <Route path="products" element={<ProtectedRoute requiredPermission="MANAGE_PRODUCTS"><ProductList /></ProtectedRoute>} />
          ...
          <Route path="customers" element={<ProtectedRoute requiredPermission="MANAGE_CUSTOMERS"><CustomersPage /></ProtectedRoute>} />
          <Route path="suppliers" element={<ProtectedRoute requiredPermission="MANAGE_SUPPLIERS"><SuppliersPage /></ProtectedRoute>} />
          <Route path="purchase-orders" element={<ProtectedRoute requiredPermission="MANAGE_PRODUCTS"><PurchaseOrdersPage /></ProtectedRoute>} />
          <Route path="expenses" element={<ProtectedRoute requiredPermission="MANAGE_EXPENSES"><ExpensesPage /></ProtectedRoute>} />
          <Route path="reports" element={<ProtectedRoute requiredPermission="VIEW_REPORTS"><FinancialReportsPage /></ProtectedRoute>} />
          <Route path="admin" element={<Navigate to="/app/admin/payments" replace />} />
          <Route path="admin/tenants" element={<ProtectedRoute requiredRole={Role.SUPER_ADMIN}><TenantManagement /></ProtectedRoute>} />
          <Route path="admin/plans" element={<ProtectedRoute requiredRole={Role.SUPER_ADMIN}><SubscriptionPlans /></ProtectedRoute>} />
          <Route path="admin/payments" element={<ProtectedRoute requiredRole={Role.SUPER_ADMIN}><PaymentApprovals /></ProtectedRoute>} />
        </Route>
